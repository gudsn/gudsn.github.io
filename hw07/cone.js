    ...
    computeVertexNormals() {
        ...
        for (let i = 0; i < vCount; i++) {
            ...
            if (len > 0) {
                ...
                const vector1 = vec3.fromValues(x, 0, z);
                const vector2 = vec3.fromValues(0, 1, 0);

                const angle = Math.atan(0.5);

                let axis = vec3.create();
                vec3.cross(axis, vector2, vector1);
                vec3.normalize(axis, axis);  // 회전 축을 정규화

                const trans = mat4.create();
                mat4.rotate(trans, trans, angle, axis);

                vec3.transformMat4(vector1, vector1, trans);

                vec3.normalize(vector1, vector1);  // 벡터를 정규화

                x = vector1[0];
                y = vector1[1];
                z = vector1[2];

                this.vertexNormals[i * 3 + 0] = x;
                this.vertexNormals[i * 3 + 1] = y;
                this.vertexNormals[i * 3 + 2] = z;
